...
const mapPriceByStatus = status => mappingPrice[status] ? mappingPrice[status] : null
const mapOriginalPriceByStatus = status => mappingOriginalPrice[status] ? mappingOriginalPrice[status] : null
...
const mapChildObj = (r, dataset) => {
    ...
    const categories = list_of_category_id
        .map(id => categoriesList.find(c => c.id === id))
    const tags = list_of_tag_id
        .map(id => tagsList.find(t => t.id === id))
    const inventory = list_of_inventory_id
...
const getOne = async e => {
    try {
        ...
        if (product) {
            ...
            const categoriesList = inListOfId.length > 0 ? await CategoryService.getList({ country_code, inListOfId }) : []
            const tagsList = inListOfTagId.length > 0 ? await TagService.getListDisplayNative(nativeLanguageCode, { inListOfTagId }) : []
            const inventoryList = await WarehouseServie.getInventoryList({listOfId: listOfInvenrotyId})
            const data = mapChildObj(product, { categoriesList, tagsList, inventoryList })
...
const getList = async e => {
    try {
        ...
        const { error: errorValidateParams, value: pararmsValid } = validateInput(inputParams, ProductSchema.GET_LIST)
        ...
        const listOfInvenrotyId = productsList.reduce((list, r) => [...list, ...r.list_of_inventory_id], [])
        const inventoryList = await WarehouseServie.getInventoryList({listOfId: listOfInvenrotyId})

        const items = productsList
            .map(r => mapChildObj(r, { categoriesList, tagsList, inventoryList }))
            .filter(r => {
                const isHasCategory = listOfCategoryIdValid.length > 0
                    ? r.list_of_category_id.some(cateId => listOfCategoryIdValid.includes(cateId))
                    : true
                return isHasCategory
            })
            .filter(r => {
                const isHasTag = listOfTagIdValid.length > 0
...
const create = async e => {
    try {
        ...
        const { error: errorValidateBody, value: validatedBody } = validateInput(body, ProductSchema.NEW_DATA)
        ...
        const inventoryResults = await InventoryService.createMultiple(newInventory)
...
const editMultiple = async e => {
    try {
        ...
        const { error: errorValidateBody, value: validatedBody } = validateInput(bodyArray, ProductSchema.EDIT_DATA_ARRAY)
        if (errorValidateBody) return createResponse(httpStatus.badRequest, { message: errorValidateBody.message })
        ...
        const editDataList = validatedBody
            .map(r => {
        ...
        const editInventory = validatedBody
            .reduce((list, r) => {
                const { inventory = [] } = r
        ...
        if (editInventory.length > 0) processExcutes.push(InventoryService.editMultiple(editInventory))
...
const remove = async e => {
    try {
        ...
        if (!product) return createResponse(httpStatus.notFound, { message: `Not found.` })
        ...
        const inventoryResults = listOfInventoryId.length > 0 
...
const getPublish = async e => {
    try {
        ...
        const [
            ...
            CategoryService.getList(pararmsValid),
            isTagNative === '1' ? TagService.getListDisplayNative(nativeLanguageCode) : TagService.getList(),
        ...
        const mapWarehouseProducts = productsList
            ...
            .reduce((list, r) => {
                // Choose inventory
                const stock = r.inventory.find(i => i.warehouse_name === warehouseValid)
                if (stock){
                    const {
                        is_enable,
                        is_allow_backorder,
        ...
        const tagOutOfStock = tagsList.find(r => r.name === systemTags.out_of_stock)

        const productWithChildren = mapWarehouseProducts
            .filter(({is_enable}) => onlyEnableValid ? is_enable : true)
            .filter(({ only_status_list = [] }) =>
                only_status_list.length > 0 ? only_status_list.includes(statusValid) : true)
            .map(r => mapChildObj(r, { categoriesList, tagsList }))
            .sort((r1, r2) => r1.product_sorting > r2.product_sorting ? 1 : r1.product_sorting < r2.product_sorting ? -1 : 0)
        ...
        const items = productWithChildren
            ...
            .map(r => toPublishData(r, { status: statusValid, tagBackOrder, tagOutOfStock }))
