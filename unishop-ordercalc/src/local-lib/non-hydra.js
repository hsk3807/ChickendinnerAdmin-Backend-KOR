...
module.exports = function (event, context) {
  ...
  async function remoteLog(message, response, url) {
    const api = Common.isLocal()
      ? 'https://member-calls2-kr-dev.unicity.com/unishop-fn-misc/log'
      : 'https://member-calls2-kr.unicity.com/unishop-fn-misc/log';
  ...
  async function getDictionary(languages) {
    ...
    const API =
      'https://member-calls2-kr.unicity.com/dictionary/publish?lang=' +
      languages.join(',');
    try {
      const result = await axios({
        method: 'GET',
  ...
  async function getProducts(
    ...
    try {
      ...
      const products = result.data.items.concat(
        result.data.renewal,
        result.data.starter_kit
      );
      return products;
    } catch (error) {
      return [];
