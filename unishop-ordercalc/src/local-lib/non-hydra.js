...
module.exports = function (event, context) {
    ...
    async function remoteLog(message, response, url) {
        const api = Common.isLocal()
            ? 'https://member-calls2-dev.unicity.com/unishop-fn-misc/log': 'https://member-calls2.unicity.com/unishop-fn-misc/log'
    ...
    async function getDictionary (languages) {
        ...
        const API = 'https://member-calls2.unicity.com/dictionary/publish?lang='+languages.join(',')
        try {
            const result = await axios({
                method: 'GET',
                url: API,
    ...
    async function getProducts (countryCode3, baCustomerType2, itemCodes = [], warehouseName = null) {
        let api = `https://member-calls2.unicity.com/products-v2/publish/${countryCode3}?status=${baCustomerType2}&onlyHasPrice=0&onlyEnable=0`
