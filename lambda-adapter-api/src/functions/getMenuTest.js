...
module.exports.handler = async (e) => {
  try {
    ...
    return createResponse(httpStatus.ok, { message: 'test1' });
    ...
    if (hydraToken) {
      return createResponse(httpStatus.ok, { test: '1' });
      // if login
      // Check Params
      ...
      return createResponse(httpStatus.ok, { test: '2' });
      // if not login
      // const menu = await MenuService.invokeGetPublishMenu({ countryCode: countryCodeValid })

      // let popup = await PopupService.invokeGetPublishMenu({ countryCode: countryCodeValid })
    ...
    return createResponse(httpStatus.ok, { test: '3' });
    console.error(err);
