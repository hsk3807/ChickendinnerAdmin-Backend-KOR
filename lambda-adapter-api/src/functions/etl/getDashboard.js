...
const getData = async e => {
  try {
    ...
    const isUseOrderKr = countryCode && USAGE_ORDER_HISTORY_KR.includes(countryCode.toUpperCase())
    const allProcesses = {
      boxProfile: {
      ...
      orderHistory: {
        func: etlService.getOrderHistoryByDashboard,
        parameter: {
          customerHref,
          tokenHydra,
          params: {
          ...
          byPassCache,
          orderType: isUseOrderKr ? "orders" : "ordersAndRmas"
          // orderType: "ordersAndRmas"
        },
      },
      // addressBook: {
    ...
    const data = JSON.stringify({...processResults, isUseOrderKr});
