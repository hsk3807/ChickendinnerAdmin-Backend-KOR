...
const getListAdmin = async (e) => {
  try {
    ...
    if (country_code) {
      ...
      } = validateInput(country_code, GeneralSchema.VALID_COUNTRY);
  ...
  try {
    ...
    const errStatus =
      code === 'ER_DUP_ENTRY'
        ? httpStatus.Conflict
        : httpStatus.InternalServerError;
    return createResponse(errStatus, {
      message: 'ㅅㄴㄷㅅㄴㄷㅅㄴㄷㅅㄴㄷㅅㄴ',
    });
  }
};
...
const edit = async (e) => {
  try {
    ...
    let input = Array.isArray(body) ? body : [body];
    ...
    const existsData = await BannerService.getList({
