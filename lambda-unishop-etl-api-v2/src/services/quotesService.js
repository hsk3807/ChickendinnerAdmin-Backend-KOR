...
const getRandomOne = async ({
    ...
    const randomOne = files.length > 0 
...
const getRandomSet = async ({
    ...
    const requestParams = Object.keys(DEVICES)
        .reduce((list, deviceKey) => {
    ...
    const processRequests = requestParams
        .map(({ countryCode, language, device }) => getRandomOne({ countryCode, language, device }))
    ...
    const mapResults = requestParams
        ...
        .reduce((obj, r) => {
            if (r.language === LANGUAGES.ENGLISH){
                ...
                obj[r.device][r.language] = r.src ? r.src : obj[r.device][LANGUAGES.ENGLISH] // use native if not exists
            }
            return obj
        },{
            mobile: {
