...
```bash
sam local invoke UshopFunction --event event.json
```
...
aws cloudformation describe-stacks \
    --stack-name sam-app \
    --query 'Stacks[].Outputs[?OutputKey==`UshopApi`]' \
    --output table
``` 
...
```bash
sam logs -n UshopFunction --stack-name sam-app --tail
```
...
* Invoke with ``sam local invoke UshopFunction --event event.json``
* Update tests
